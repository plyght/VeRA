    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        if data.len() < 4 {
            return Err(VeraError::CorruptedData(
                "Tile index too small".to_string()
            ));
        }
...
impl<R: Read + Seek> Container<R> {
    ...
    fn load_metadata(&mut self) -> Result<()> {
        self.inner.seek(SeekFrom::Start(self.header.metadata_offset))?;
    ...
    fn load_tile_index(&mut self) -> Result<()> {
        self.inner.seek(SeekFrom::Start(self.header.tile_index_offset))?;
    ...
    pub fn read_vector_data(&mut self) -> Result<Vec<u8>> {
        self.inner.seek(SeekFrom::Start(self.header.vector_offset))?;
